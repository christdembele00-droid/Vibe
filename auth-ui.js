...
let modal = null;
const ERROR_MESSAGES = {'auth/popup-closed-by-user':'La fenêtre de connexion a été fermée.','auth/popup-blocked':'La fenêtre de connexion a été bloquée. Utilisez la redirection.','auth/account-exists-with-different-credential':'Ce compte existe déjà avec un autre mode de connexion.','auth/unauthorized-domain':'Ce domaine doit être autorisé dans Firebase Authentication.','auth/operation-not-allowed':'Ce moyen de connexion n’est pas activé dans Firebase.','auth/network-request-failed':'Connexion réseau impossible. Vérifiez votre connexion puis réessayez.','auth/invalid-credential':'Identifiants invalides.','auth/invalid-email':'Adresse e-mail invalide.','auth/weak-password':'Le mot de passe doit comporter au moins 6 caractères.','auth/email-already-in-use':'Cette adresse e-mail est déjà utilisée.','auth/invalid-login-credentials':'E-mail ou mot de passe incorrect.','auth/user-not-found':'Aucun compte ne correspond à cette adresse e-mail.','auth/wrong-password':'E-mail ou mot de passe incorrect.','auth/too-many-requests':'Trop de tentatives. Réessayez plus tard.','auth/internal-error':'Le service de connexion a rencontré une erreur. Réessayez.'};
...
function ensureModal(){if(modal)return modal;modal=document.createElement('div');modal.className='auth-modal';modal.hidden=true;modal.innerHTML=`<div class="auth-card" role="dialog" aria-modal="true" aria-labelledby="authTitle"><button class="glass-btn auth-close" id="authClose" type="button" aria-label="Fermer">×</button><h2 id="authTitle">Connexion à Vibe</h2><p id="authCopy">Choisissez votre moyen de connexion.</p><div class="auth-actions"><button type="button" id="googleLogin">Continuer avec Google</button><button type="button" id="githubLogin">Continuer avec GitHub</button><button type="button" id="emailMode">Continuer avec e-mail</button><form id="emailForm" hidden><input id="authEmail" type="email" placeholder="Adresse e-mail" autocomplete="email"><input id="authPassword" type="password" placeholder="Mot de passe" autocomplete="current-password"><button type="submit" id="emailLogin">Se connecter</button><button type="button" id="emailRegister">Créer un compte</button></form><button type="button" id="logoutBtn" hidden>Se déconnecter</button></div><div class="auth-error" id="authError" role="alert" aria-live="polite"></div></div>`;document.body.appendChild(modal);$('#authClose').onclick=closeLogin;modal.addEventListener('click',e=>{if(e.target===modal)closeLogin()});$('#googleLogin').onclick=()=>void loginWithProvider(googleProvider);$('#githubLogin').onclick=()=>void loginWithProvider(githubProvider);$('#emailMode').onclick=()=>{$('#emailForm').hidden=!$('#emailForm').hidden};$('#emailForm').onsubmit=e=>{e.preventDefault();void emailAuth(false)};$('#emailRegister').onclick=()=>void emailAuth(true);$('#logoutBtn').onclick=async()=>{try{await signOut(auth);closeLogin()}catch(e){showError(e)}};return modal}
async function loginWithProvider(provider){const error=$('#authError');if(error)error.textContent='';try{await signInWithPopup(auth,provider);closeLogin()}catch(e){if(e?.code==='auth/popup-blocked'){try{await signInWithRedirect(auth,provider)}catch(redirectError){showError(redirectError)}}else{showError(e)}}}
async function emailAuth(register){const error=$('#authError');if(error)error.textContent='';const email=$('#authEmail')?.value.trim();const password=$('#authPassword')?.value||'';if(!email)return showError({code:'auth/invalid-email'});if(!password)return showError({code:'auth/invalid-login-credentials'});try{const result=register?await createUserWithEmailAndPassword(auth,email,password):await signInWithEmailAndPassword(auth,email,password);if(register&&result.user&&!result.user.displayName)await updateProfile(result.user,{displayName:email.split('@')[0]});closeLogin()}catch(e){showError(e)}}
