...
async function publishStatus() {
  ...
  try {
    ...
    await addDoc(collection(db, 'statuses'), {
      ...
      authorId: user.uid,
      authorName: profileData.name || 'Utilisateur Vibe',
...
async function createChannel() {
  const user = auth?.currentUser;
  if (!user || !db) {
    showToast('Connectez-vous pour créer une chaîne.');
    return;
  }

  const name = String(prompt('Nom de la chaîne :') || '').trim().slice(0, 60);
  if (!name) return;
  ...
  try {
    ...
    const channelRef = await addDoc(collection(db, 'channels'), {
      ...
      description,
      ownerId: user.uid,
      ownerName: profileData.name || 'Utilisateur Vibe',
    ...
    await setDoc(doc(db, 'channelFollowers', `${channelRef.id}_${user.uid}`), {
    ...
    showToast('Impossible de créer la chaîne.');
  }
}
...
async function toggleChannelFollow(channel, button) {
  const user = auth?.currentUser;
  if (!user || !db) {
    showToast('Connectez-vous pour suivre une chaîne.');
    return;
  }
  ...
  try {
    ...
    if (existing.exists()) {
      await setDoc(followRef, { unfollowedAt: serverTimestamp(), active: false }, { merge: true });
      button.textContent = 'Suivre';
      showToast('Chaîne retirée de vos abonnements.');
    } else {
      ...
      showToast('Chaîne suivie.');
    }
  } catch (error) {
    console.error('[Vibe] Abonnement chaîne:', error);
    showToast('Impossible de modifier l’abonnement.');
  }
}
...
async function publishChannelPost(channel) {
  const user = auth?.currentUser;
  if (!user || !db) return;
  const text = String(prompt(`Publication dans « ${channel.name} » :`) || '').trim().slice(0, 1000);
  if (!text) return;
  ...
  try {
    ...
    showToast('Impossible de publier.');
  }
}
...
function loadChannelPosts(channel, container) {
  ...
  return onSnapshot(postsQuery, snapshot => {
    ...
    container.innerHTML = posts.map(post => `
      <article class="status-card">
        <div class="feature-avatar">${escapeHtml((post.authorName || channel.name || 'V').slice(0, 1).toUpperCase())}</div>
        <div class="status-body">
          <strong>${escapeHtml(post.authorName || channel.name)}</strong>
...
async function openChannel(channel) {
  ...
  showPanel(panelShell(
    channel.name,
    channel.description || `Chaîne Vibe · ${channel.followersCount || 0} abonnés`,
    `<div class="feature-card">
      <div class="feature-card-title"><span class="feature-icon">📢</span><div><h3>${escapeHtml(channel.name)}</h3><p>${escapeHtml(channel.ownerName || 'Créateur Vibe')}</p></div></div>
      <div class="feature-actions">
        <button class="secondary-btn" id="channel-follow-btn" type="button">Suivre</button>
        ${channel.ownerId === auth?.currentUser?.uid ? '<button class="primary-btn" id="channel-publish-btn" type="button">Publier</button>' : ''}
      </div>
    </div>
    <div class="feature-card">
      <div class="feature-card-title"><div><h3>Publications</h3><p>Les dernières publications de la chaîne.</p></div></div>
      <div id="${postsId}" class="status-list"><div class="feature-empty">Chargement...</div></div>
    </div>`
  ));
  ...
  const container = document.getElementById(postsId);
  if (container) stopChannels = loadChannelPosts(channel, container);
}
...
function loadChannels() {
  ...
  stopChannels = onSnapshot(channelsQuery, snapshot => {
    ...
    container.innerHTML = channels.map(channel => `
      <article class="call-row channel-row" data-channel-id="${escapeHtml(channel.id)}">
        <div class="feature-avatar">${escapeHtml((channel.name || 'V').slice(0, 1).toUpperCase())}</div>
        <div class="call-main"><strong>${escapeHtml(channel.name || 'Chaîne Vibe')}</strong><p>${escapeHtml(channel.description || 'Chaîne Vibe')} · ${channel.followersCount || 0} abonnés</p></div>
        <button class="secondary-btn channel-open-btn" type="button">Ouvrir</button>
      </article>`).join('');

    container.querySelectorAll('.channel-open-btn').forEach(button => {
...
function openChannels() {
  ...
  showPanel(panelShell(
    'Chaînes',
    'Découvrez, suivez et publiez sur les chaînes Vibe.',
    `<div class="feature-card call-hero">
      <div class="call-hero-icon">📢</div>
      <h3>Chaînes Vibe</h3>
      <p>Suivez les chaînes qui vous intéressent et créez votre propre espace de publication.</p>
      <div class="feature-actions feature-actions-center">
        <button class="primary-btn" id="create-channel-btn" type="button">+ Créer une chaîne</button>
      </div>
    </div>
    <div class="feature-card">
      <div class="feature-card-title"><div><h3>Découvrir</h3><p>Les chaînes disponibles sur Vibe.</p></div></div>
      <div id="channel-list-container" class="calls-list"><div class="feature-empty">Chargement des chaînes...</div></div>
    </div>`
  ));
...
export function initWhatsAppNavigation() {
  ...
  if (btnChannels && !btnChannels.dataset.vibeBound) {
    btnChannels.dataset.vibeBound = 'true';
    btnChannels.addEventListener('click', openChannels);
  }
