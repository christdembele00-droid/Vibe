...
$('#backBtn').addEventListener('click',()=>{chatPanel.classList.remove('open');if(unsubscribeMessages)unsubscribeMessages();if(unsubscribeTyping)unsubscribeTyping();unsubscribeMessages=null;unsubscribeTyping=null;selectedId=null;});
...
$('#attachBtn').addEventListener('click',()=>{const input=$('#fileInput');if(input)input.click();});
$('#fileInput').addEventListener('change',e=>{const file=e.target.files?.[0];if(file)showToast(`Fichier sélectionné : ${file.name}`);e.target.value='';});
