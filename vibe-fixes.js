...
function decorateReactions(data) {
  const map = {};
  for (const [messageId, users] of Object.entries(data || {})) {
    const counts = {};
    for (const value of Object.values(users || {})) counts[value] = (counts[value] || 0) + 1;
    map[messageId] = Object.entries(counts).map(([emoji, count]) => emoji + (count > 1 ? ` ${count}` : '')).join(' ');
  }
  const elements = [...document.querySelectorAll('#messages [data-message]')];
  let index = 0;
  const paintChunk = () => {
    const end = Math.min(index + 20, elements.length);
    ...
    if (index < elements.length) setTimeout(paintChunk, 0);
  };
  paintChunk();
}
function watchReactions(id) { stopReactions?.(); const q = query(collection(db, 'conversations', id, 'reactions'), limit(500)); stopReactions = onSnapshot(q, (snap) => { const data = {}; snap.forEach((item) => { const d = item.data(); if (!data[d.messageId]) data[d.messageId] = {}; data[d.messageId][d.uid] = d.emoji; }); decorateReactions(data); }, () => {}); }
