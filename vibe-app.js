...
async function toggleFavorite(chat,event){
  ...
  }catch(error){favoriteChatIds=previous;allChats=sortChats(allChats);render(allChats);console.error('[Vibe] Favori:',error);showToast('Impossible de modifier le favori.')}
}

async function supprimerDiscussion(chat,event){
  event?.preventDefault();
  event?.stopPropagation();
  if(!currentUser||!db||!chat?.id||chat.id==='general')return;
  const confirmation=window.confirm(`Supprimer la discussion « ${chat.name||'Discussion'} » ?`);
  if(!confirmation)return;
  try{
    await deleteDoc(doc(db,'chats',chat.id));
    favoriteChatIds.delete(chat.id);
    allChats=allChats.filter(item=>item.id!==chat.id);
    render(allChats);
    shell?.classList.remove('chat-open');
    showToast('Discussion supprimée.');
  }catch(error){
    console.error('[Vibe] Suppression discussion:',error);
    showToast('Impossible de supprimer cette discussion.');
  }
}

function annulerAppuiLong(button){
  const timer=longPressTimers.get(button);
  if(timer){clearTimeout(timer);longPressTimers.delete(button)}
}

function activerAppuiLong(button,item){
  let moved=false;
  const start=event=>{
    if(event.pointerType==='mouse'&&event.button!==0)return;
    moved=false;
    annulerAppuiLong(button);
    const timer=setTimeout(()=>{
      longPressTimers.delete(button);
      if(!moved&&item.id!=='general')supprimerDiscussion(item,event);
    },650);
    longPressTimers.set(button,timer);
  };
  button.addEventListener('pointerdown',start);
  button.addEventListener('pointerup',()=>annulerAppuiLong(button));
  button.addEventListener('pointercancel',()=>annulerAppuiLong(button));
  button.addEventListener('pointerleave',()=>annulerAppuiLong(button));
  button.addEventListener('pointermove',()=>{moved=true;annulerAppuiLong(button)});
}
...
function render(items=allChats){
  ...
  for(const item of sortChats(items)){
    ...
    button.type='button';button.className='chat-item';button.dataset.chatId=item.id;
    ...
    activerAppuiLong(button,item);
    list.appendChild(button);
  }
  applySearch(search?.value||'');
}

async function ensureGeneralChat(){if(!db||!currentUser)return;const reference=doc(db,'chats','general');const snapshot=await getDoc(reference);if(!snapshot.exists())await setDoc(reference,{name:'Discussion générale',lastMessage:'Bienvenue',lastUpdated:serverTimestamp(),type:'general'})}

async function enregistrerUtilisateurActif(user,profile=null){
  if(!user||!db)return;const data=profile||{};const name=user.displayName||data.name||'Utilisateur';const photoURL=user.photoURL||data.photoURL||'';const vibeId=data.vibeId||`vibe-${user.uid.slice(-8).toLowerCase()}`;
  try{await setDoc(doc(db,'users',user.uid),{uid:user.uid,name,displayName:name,photoURL,vibeId,email:user.email||data.email||'',lastSeen:serverTimestamp(),online:true},{merge:true});const currentAvatar=document.getElementById('current-user-avatar');if(currentAvatar){const avatar=creerAvatarDepuisProfil({name,photoURL},{className:'user-avatar'});avatar.id='current-user-avatar';avatar.style.width='40px';avatar.style.height='40px';avatar.style.minWidth='40px';currentAvatar.replaceWith(avatar)}const userName=document.getElementById('current-user-name');if(userName)userName.textContent=name}catch(error){console.error('[Vibe] Présence utilisateur:',error)}}
...
function startChatsListener(){
  if(!db||!currentUser)return;stopChats?.();stopChats=null;
  const privateChatsQuery=query(collection(db,'chats'),where('participantIds','array-contains',currentUser.uid));
  const stopPrivateChats=onSnapshot(privateChatsQuery,snapshot=>{const privateChats=snapshot.docs.map(item=>({id:item.id,...item.data()})).filter(item=>item.id!=='general');const general=allChats.find(item=>item.id==='general')||fallbackChats[0];allChats=[general,...privateChats];render(allChats)},error=>{console.error('[Vibe] Conversations privées:',error);allChats=[fallbackChats[0]];render(allChats)});
  const stopGeneral=onSnapshot(doc(db,'chats','general'),snapshot=>{const general=snapshot.exists()?{id:snapshot.id,...snapshot.data()}:fallbackChats[0];const privateChats=allChats.filter(item=>item.id!=='general');allChats=[general,...privateChats];render(allChats)},error=>console.error('[Vibe] Discussion générale:',error));
  stopChats=()=>{stopPrivateChats?.();stopGeneral?.()};
}

async function loadCurrentProfile(user){
  if(!db||!user)return null;try{const profile=await ensureVibeProfile();const name=user.displayName||profile?.name||'Utilisateur';const photoURL=user.photoURL||profile?.photoURL||'';const currentAvatar=document.getElementById('current-user-avatar');const userName=document.getElementById('current-user-name');if(currentAvatar){const avatar=creerAvatarDepuisProfil({name,photoURL},{className:'user-avatar'});avatar.id='current-user-avatar';avatar.style.width='40px';avatar.style.height='40px';avatar.style.minWidth='40px';currentAvatar.replaceWith(avatar)}if(userName)userName.textContent=name;return {...profile,name,displayName:name,photoURL,email:user.email||''}}catch(error){console.error('[Vibe] Profil initial:',error);return {name:user.displayName||'Utilisateur',displayName:user.displayName||'Utilisateur',photoURL:user.photoURL||'',email:user.email||''}}}
