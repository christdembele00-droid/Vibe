...
function renderReactions(article, rows) {
  ...
  rows.forEach(row => {
    if (row.emoji) counts.set(row.emoji, (counts.get(row.emoji) || 0) + 1);
  });
  const bar = document.createElement('div');
  bar.className = 'vibe-reactions';
  bar.innerHTML = [...counts.entries()]
    .map(([emoji, count]) => `<span title="${count} réaction(s)">${escapeHtml(emoji)} <b>${count}</b></span>`)
    .join('');
...
function enhanceMessage(article) {
  ...
  getDoc(doc(db, 'conversations', chatId, 'messages', messageId)).then(snap => {
    ...
    if (mime.startsWith('image/')) {
      bubble.innerHTML = `<img class="message-media" src="${escapeHtml(url)}" alt="${escapeHtml(message.fileName || 'Image')}" loading="lazy">`;
    } else if (mime.startsWith('video/')) {
      bubble.innerHTML = `<video class="message-media" src="${escapeHtml(url)}" controls preload="metadata"></video>`;
    } else {
      bubble.innerHTML = `<a class="message-file" href="${escapeHtml(url)}" target="_blank" rel="noopener" download="${escapeHtml(message.fileName || 'fichier')}">📎 ${escapeHtml(message.fileName || 'Fichier')}</a>`;
    }
  }).catch(() => {});
}
