...
async function hardResetClientSession(): Promise<void> {
  ...
  try {
    const databases = (indexedDB as IDBFactory & { databases?: () => Promise<IDBDatabaseInfo[]>; }).databases;
    if (databases) {
      const entries = await databases.call(indexedDB);
      await Promise.all(
        entries.map(entry => entry.name).filter((name): name is string => Boolean(name)).map(
...
export async function api<T>(path: string, options: ApiOptions = {}): Promise<T> {
  ...
  if (!baseUrl) {
    throw new Error("Serveur VIBE non configuré. Définis NEXT_PUBLIC_API_URL pour la version en ligne.");
  }
  ...
  if (!response.ok) {
    ...
    if (response.status === 401) await hardResetClientSession();
