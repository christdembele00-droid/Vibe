...
class ConnectionManager:
    ...
    def disconnect(self, key: str, ws: WebSocket):
        peers = self.connections.get(key)
        if not peers:
            return
        peers.discard(ws)
        if not peers:
...
async def conversation_socket(websocket: WebSocket, conversation_id: UUID):
    ...
    try:
        ...
        decoded = verify_bearer_token("Bearer " + token if token else None)
        user = get_local_user_by_firebase_uid(str(decoded["uid"]))
        if not user or user.get("deleted_at") is not None:
            raise ValueError("user")
        with get_engine().connect() as conn:
            allowed = conn.execute(
                text("SELECT 1 FROM conversation_members WHERE conversation_id=:c AND user_id=:u AND left_at IS NULL"),
                {"c": conversation_id, "u": user["id"]},
            ).first()
        if not allowed:
