...
function openModal(title, body) {
  ...
  modal.querySelector('header button').onclick=closeModal;
...
async function publishStatus() {
  ...
  try{
    ...
    toast('Statut publié pour 24 h.');closeModal();
...
function statusUI(){
  ...
  const stop=onSnapshot(collection(db,'statuses'),snap=>{
    const now=Date.now();
  ...
  },e=>{console.warn('[Vibe] statuts',e);const list=$('vibeStatusList');if(list)list.textContent='Impossible de charger les statuts.';});
  modal?.querySelector('header button')?.addEventListener('click',stop,{once:true});
}
...
async function handleMedia(e){
  ...
  if(!file||!uid())return;
  const cid=currentChatId();if(!cid)return toast('Ouvrez une discussion.');
  try{
...
async function voice(){
  ...
  $('vibeVoiceStart').onclick=async()=>{try{const stream=await navigator.mediaDevices.getUserMedia({audio:true});chunks=[];recorder=new MediaRecorder(stream);recorder.ondataavailable=e=>e.data.size&&chunks.push(e.data);recorder.onstop=async()=>{stream.getTracks().forEach(t=>t.stop());try{const blob=new Blob(chunks,{type:recorder.mimeType||'audio/webm'});const file=new File([blob],`vocal-${Date.now()}.webm`,{type:blob.type||'audio/webm'});const url=await uploadMedia(file,'voice');await addDoc(collection(db,'conversations',cid,'messages'),{uid:uid(),text:'🎙️ Message vocal',type:'media',dataUrl:url,mediaUrl:url,mimeType:file.type,fileName:file.name,fileSize:file.size,createdAt:serverTimestamp(),viewOnce:false});toast('Message vocal envoyé.');closeModal();}catch(e){toast(`Envoi vocal impossible : ${e.message}`)}};recorder.start();$('vibeVoiceStart').disabled=true;$('vibeVoiceStop').disabled=false;$('vibeVoiceState').textContent='Enregistrement en cours…';}catch(e){toast('Accès au microphone refusé ou indisponible.')}};
  $('vibeVoiceStop').onclick=()=>{if(recorder?.state==='recording')recorder.stop();};
}

function renderReactions(article,rows){article.querySelector('.vibe-reactions')?.remove();if(!rows.length)return;const counts=new Map();rows.forEach(x=>{if(x.emoji)counts.set(x.emoji,(counts.get(x.emoji)||0)+1)});const bar=document.createElement('div');bar.className='vibe-reactions';bar.innerHTML=[...counts.entries()].map(([emoji,count])=>`<button type="button" title="${count} réaction(s)">${esc(emoji)} <span>${count}</span></button>`).join('');article.appendChild(bar);}
...
function enhanceMessage(article){
  ...
  getDoc(doc(db,'conversations',cid,'messages',id)).then(snap=>{
    ...
    if(mime.startsWith('image/'))bubble.innerHTML=`<img class="message-media" src="${esc(url)}" alt="${esc(m.fileName||'Image')}" loading="lazy">`;
    else if(mime.startsWith('video/'))bubble.innerHTML=`<video class="message-media" src="${esc(url)}" controls preload="metadata"></video>`;
    else if(mime.startsWith('audio/'))bubble.innerHTML=`<audio class="message-audio" src="${esc(url)}" controls preload="metadata"></audio>`;
    else bubble.innerHTML=`<a class="message-file" href="${esc(url)}" target="_blank" rel="noopener" download="${esc(m.fileName||'fichier')}">📎 ${esc(m.fileName||'Fichier')}</a>`;
  }).catch(()=>{});
}

function observeMessages(){const box=$('messages');if(!box)return;const scan=()=>box.querySelectorAll('.message').forEach(enhanceMessage);scan();new MutationObserver(scan).observe(box,{childList:true,subtree:true});}
...
function bind(){
  ...
  const bar=document.createElement('div');bar.className='vibe-extra-bar';bar.innerHTML='<button type="button" data-extra="status">◉ Statuts</button><button type="button" data-extra="voice">🎙️ Vocal</button>';document.querySelector('.main-tabs')?.after(bar);
