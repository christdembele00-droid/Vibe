...
const replaceNewChatButton = () => {
  const old=document.getElementById('newChatBtn');
  if(!old || old.dataset.vibeDirectChat==='true') return;
  const fresh=old.cloneNode(true);
  fresh.dataset.vibeDirectChat='true';
  old.replaceWith(fresh);
  fresh.addEventListener('click',createDirectChat);
};
const createDirectChat = async () => {
  ...
  try{
    ...
    if(found){toast('Cette discussion existe déjà.');document.dispatchEvent(new CustomEvent('vibe:open-chat',{detail:{chat:found}}));return;}
...
replaceNewChatButton();
document.addEventListener('vibe:auth-changed',replaceNewChatButton);
